...
export async function POST(req: NextRequest) {
  try {
    ...
    const docTypeLabel = preparedDoc.type === 'final-onboarding'
      ? 'Final Onboarding Document'
      : (preparedDoc.type ? preparedDoc.type.charAt(0).toUpperCase() + preparedDoc.type.slice(1).replace(/-/g, ' ') : 'Document')
    ...
    if (isBundle) {
      ...
      const invoiceLabel = preparedDoc.type === 'pre-invoice'
        ? 'Pre-invoice'
