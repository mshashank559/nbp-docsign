...
export async function POST(req: NextRequest) {
  try {
    ...
    if (accessToken && process.env.GMAIL_SENDER_EMAIL) {
      const msg = buildMime({ from: process.env.GMAIL_SENDER_EMAIL, to: doc.client_email, subject: doc.type === 'agreement' ? 'NetBonds Signed Agreement Executive Copy' : `Your signed ${docLabel} — NetBounce Placement LLC`, html })
